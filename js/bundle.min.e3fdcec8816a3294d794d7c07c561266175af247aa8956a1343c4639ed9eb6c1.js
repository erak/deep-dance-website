...
nextColor=0;else
nextColor++;}
$(document).mousemove(function(event){var mouseDiffX=Math.abs(mouseX-event.pageX);var mouseDiffY=Math.abs(mouseY-event.pageY);if(mouseDiffX>mouseThreshold||mouseDiffY>mouseThreshold){changeOverlay();}
mouseX=event.pageX;mouseY=event.pageY;});$(document).scroll(function(event){changeOverlay();});$(document).ready(function(){resizeOverlay();setInterval(function(){changeBackgroundColor();},100);preload(overlays);});$(window).resize(function(){resizeOverlay();});;(function(){'use strict';if(!document.queryCommandSupported('copy')){return;}
